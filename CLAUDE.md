...
python3 scripts/build.py install # copy into the SC2 GAME INSTALL dir (C:\Program Files (x86)\StarCraft II = CCM's sc2BasePath) — NOT Documents
```
...
- **Verify the mod is actually LOADED before debugging the code.** "Nothing changed" almost always means the build isn't being loaded, not that the code is wrong: `build.py install` targets the SC2 **game install dir** (`C:\Program Files (x86)\StarCraft II` — CCM's `sc2BasePath`, the SC2.exe directory from the registry; **NOT** `Documents\StarCraft II`), because that's where CCM + the game read custom campaigns/mods from (`sc2BasePath\Maps\Campaign` + `\Mods`; verified in CCM's source, github.com/7thAce/SC2CCM). The in-mission **canary subtitle** (or a static load-proof) confirms the library ran. Check WHERE the files land + whether the canary shows FIRST — a byte-correct mod installed to the wrong place looks identical to a broken mod (installing to Documents was the v0.3.14–v0.3.18 spiral: CCM reported "no mod installed"; see [docs/learnings.md](docs/learnings.md) §Verification tooling).
...
- **Fix the bug CLASS, not just the reported instance (owner standing rule).** When you find or fix a bug, IMMEDIATELY check whether the same root cause hits any other unit/weapon/ability/clone/card and fix EVERY instance in the same change — never patch the one reported case and wait for the siblings to be re-reported. Almost every bug here is a class: a weapon-`Effect` reroute that detaches the bound muzzle/projectile actor (Thor → Viking → Wraith…), a merc silently missing a base-unit buff, a wrong merge-unaware card index, a guessed id. Enumerate the class (grep the pattern across all units/weapons/clones), fix all, and state in the report which other candidates you checked and the result. **Every fix dispatch brief must carry an explicit "sweep for the same bug class and fix every instance" instruction.**
...
- **A release MUST also ship the Nightmare variant zip WHEN the Nightmare base is provided locally** (`mods/Nightmare/extracted/`). CI only builds the **standard** zip (it has no third-party inputs), so the Nightmare zip is a local build + manual upload — easily forgotten. **Enforced (not just described):** the `pre-push` hook ([scripts/hooks/pre-push](scripts/hooks/pre-push)) BLOCKS pushing a `v<version>` tag if the base is present but `dist/WoL-Unbalanced-Nightmare-v<version>.zip` isn't built; `scripts/check_release_assets.py` verifies the PUBLISHED release carries both assets (run it as the last release step). When the base is absent (fresh clone / CI), Nightmare is not required. **Release procedure when the base is present:** bump `VERSION` → `python3 scripts/build.py package nightmare` → commit + push `main` → push the `v*` tag (hook passes since the zip exists) → after CI publishes, `gh release upload v<version> dist/WoL-Unbalanced-Nightmare-v<version>.zip` → `python3 scripts/check_release_assets.py` (must pass). Wire the hook on a fresh clone with `scripts/hooks/install.sh`.
