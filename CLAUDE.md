...
- **Third-party / generated content is excluded by default** (allowlist model): `mods/`, `vendor/`, `build/`, `dist/`, `.claude/`, compiled `tools/mpqpatch` are gitignored. A third-party item may be committed/bundled ONLY if it is **explicitly allowlisted in [docs/third-party-allowlist.md](docs/third-party-allowlist.md) with its license on file** permitting redistribution (then add a `!`-negation in `.gitignore`). Nothing is allowlisted yet — Rhyme's Nightmare pack has NO license file, so it is NOT eligible until its author grants/publishes redistribution terms. Until then the Nightmare variant is **build-it-yourself** (ship the build mode, not the artifact). Never commit large binaries (zips, maps) into git history — release artifacts go to GitHub *release assets* (stored outside the repo), not the repo.
