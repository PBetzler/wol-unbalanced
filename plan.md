...
**M2 next:** a generator script (`scripts/genlib.py`) that reads the reference XML + our change list and emits the `libWoLU_ApplyPlayer` body — starting with the global rules (damage-point cap, free cloak, build-time cap, Marine +20) followed by armory/lab auto-grant and merc/hero parity.
