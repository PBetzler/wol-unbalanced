...
**Still BLOCKED / deliberately NOT guessed (no clean blueprint anywhere — Liberty/NCO/AP/WOLRE/Moebius all checked):** Tank Graduating Range (stacking range-on-siege), Wraith Trigger Override (stacking attack-speed-on-attack — Moebius has no MaxStackCount-on-attack pattern), Predator Charge/Vespene Synthesis, Viking Shredder *line* splash / Wild Missiles salvo, Hellbat Aspect morph (needs HotS assets), SV Magellan/EMP Shockwave, Medic Restoration/Optical Flare, full smart-Snipe priority, BC smart-batteries (multi-target anti-overkill). These need either a galaxy targeting/stacking trigger (a new mini-framework) or assets we don't have.
