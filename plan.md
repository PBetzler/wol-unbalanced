...
**Ability visuals — pattern established + Magrail VERIFIED in game (2026-06-13):** the explicit-index append fix made all appended abilities appear; the save-version error is inherent to mid-mission saves (test fresh, not reload — see learnings). Visual actors bind to ability/weapon/effect IDs, so clones reusing vanilla EFFECT ids keep their projectile/beam/impact visuals (EMP/Snipe/Yamato/Obliterate ✓); only ability-bound caster animations (stim puff, cast poses, Yamato charge glow) are cosmetically absent. Magrail (no vanilla source) now renders the **authentic WarHound railgun beam + impact**, VERIFIED in game — via the reusable pattern: **reference the original's base-CASC `.m3` by path (CModel) + clone the original's actor + match model names exactly** (a `<Model value="X"/>`/`CModel id="X"` mismatch silently yields no visual — that was the beam bug). No bundling/dependency needed for base-game effect models (faction-swap examples in `mods/` confirm: they bundle only premium skins, depend only for whole units). Thor de-cloned (vanilla weapon id kept → attack animation fires; splash via effect-override).

**Parity + completeness sweep + visible-upgrade icons (2026-06-14, overnight batch — commits 790f8b7, edde40b, 2490804, 8e495ec).** Mercs/heroes use DISTINCT weapon ids, so base-unit weapon edits never reached them (the "Marine vs War Pigs" class of bug). Mirrored every headline buff onto its merc + hero counterpart and filled base-unit Laser-Targeting gaps — all per-player, all via `Add`/`Set` so robust to each weapon's base. Covered: Marine→War Pigs+Raynor×3 (range+sight), Marauder→Hammer Securities+Swann (range/sight/+2 armor), Firebat→Devil Dogs+Tychus (×2 HP +100 Kinetic Foam; Devil Dog flamer +2 range), Ghost→Nova, Spectre→Tosh, Goliath→Spartan Company (Ares-Class +3/+1, Shaped Hull +25), Siege Tank→Siege Breakers (range/sight/+3 sieged armor), Banshee→Dusk Wings, Thor→Odin (AA range/dmg + ground range/sight), BC→Jackson's Revenge (fire-while-moving). Base LTS gaps filled for Reaper/Siege Tank/Thor/Banshee. **Added Nova + Tosh ability kits** (Stim/Snipe/EMP and Stim/Psi-Lash) and **SCV/Vulture AP ports** (faster attack; fire-while-moving). **Visible upgrades:** passive `Type=Passive` card icons (`WoLUHaveFlag`-gated) on Marine + Marine-line mercs/heroes + Marauder/Reaper/Hellion/Goliath/Siege Tank/Thor/Viking/Banshee/Medivac/Ghost/Spectre/BC (Goliath→Shaped Hull+Opt-Logistics, BC→Internal-Tech+Opt-Logistics — neither has LTS). **Bug fixed in this sweep:** SpartanCompany was getting Shaped Hull twice (×1.2 multiply AND flat +25) → removed the redundant multiply. **Audit (automated):** all 174 genlib stat-edit entries resolve against reference catalogs; all `Face`/`AbilCmd`/`AbilArray` links in UnitData resolve; Magrail actor↔model wiring consistent; lint+build+install green.
