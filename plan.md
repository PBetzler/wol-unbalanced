...
**Open next:** in-game verification of M2+M3+parity; WP-C remaining AP ports (abilities/behaviors: Magrail, Reaper cloak+flight, Medivac cloak, Wraith trigger override/laser tech, Viking missiles, Banshee cloak-damage perks, Hellion morphs, Tank graduating range/spider mines, Predator kit, Science Vessel kit, Raven turrets); WP-D customs (Raven PDD, BC batteries/Yamato, smart Snipe); Ghost "Resource Efficiency (AP)" exact values unverified (only Bargain Bin applied); Odin AA damage left untouched (base value unknown — don't nerf). Risky/unverified field paths to watch in game: `Charge.*`/`Cooldown.TimeStart` on train infos, `Button.Requirements` swap, `AreaArray[n]` creation on ThorsHammerDamage, `Cost[0].Vital[Energy]` on cloak abils, negative stim cost, AbilArray/LayoutButtons creation on heroes, Medivac `TotalCargoSpace`.
