...
- **Graduating Range is NOT in the rogue kit** (no data, no string, no galaxy siege-range code; `Kit@ImprovedSiegeMode` = +3 armor sieged + faster transform). User's pointer didn't pan out — need the actual source mod, or build it from primitives (periodic stacking WeaponRange buff while sieged).

**Still BLOCKED / deliberately NOT guessed:** Tank Graduating Range (buildable from primitives but needs in-game tuning), Wraith Trigger Override (stacking attack-speed-on-attack), Predator Charge/Vespene Synthesis, Viking Shredder *line* splash / Wild Missiles salvo, Hellbat Aspect morph (needs HotS assets), SV Magellan/EMP Shockwave, Medic Restoration/Optical Flare, BC smart-batteries (blueprinted, needs in-game iteration). Next architectural decision: build a small galaxy targeting/stacking helper, or transplant the rogue kit's data systems with playtesting.
