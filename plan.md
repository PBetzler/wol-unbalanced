...
**BLOCKED — needs the "Moebius mod" the user mentioned (NOT FOUND on disk — searched full SC2 tree, Downloads, Desktop, Documents, /tmp, Blizzard/Battle.net cache, iCloud, full home):** the genuinely-custom mechanics with no Liberty/NCO/AP blueprint — Tank Graduating Range (stacking range-on-siege), Wraith Trigger Override (stacking attack-speed-on-attack), Predator Charge/Adaptive Defenses/Vespene Synthesis, Viking Shredder *line* splash / Wild Missiles salvo, Hellbat Aspect morph (also needs HotS assets), SV Magellan/EMP Shockwave/Tactical Jump, Medic Restoration/Optical Flare, full smart-Snipe priority, BC smart-batteries (multi-target anti-overkill). These were deliberately NOT guessed.
