...
**Open next:** in-game verification of M2+M3; WP-C ported AP upgrades per unit; WP-D customs (Raven PDD, BC batteries/Yamato, smart Snipe); hero parity (WP-A.4); merc %-parity recompute on top of new stats (WP-A.5); Ghost "Resource Efficiency (AP)" exact values still unverified (only Bargain Bin applied). Risky/unverified field paths to watch in game: `Charge.*`/`Cooldown.TimeStart` on train infos, `Button.Requirements` swap, `AreaArray[n]` creation on ThorsHammerDamage, `Cost[0].Vital[Energy]` on cloak abils.
