...
| **Viking** | 150/75, 2 supply | All AP; Ripwave; Phobos (+range); **Shredder line-splash** + Anti-Mech (Assault mode); +55% speed (Aesir Turbines); Smart Servos; **free permanent cloak (0.3.11, owner request — both modes)**; **now scales (air+ground) weapons + armor with the combined Eng Bay upgrade** (0.3.13 added base Viking entries to the base-Level upgrades) | ✅ ground splash + anti-mech; **ground projectile/impact RESTORED 0.3.13** (the `TwinGatlingCannon`→`*WoLUSet` Effect reroute had detached `VikingAssaultAttack`'s launch sound + impact sound + blood; rebound per validator branch `VikingAssaultAttack{Vanilla,}WoLU` + merc `WreckingCrewAssaultAttackWoLU` — the prior "ground doesn't hit/scale" report was this missing visual, not a real damage gap; 🟡 [GAME] confirm impact/blood shows); ✅ **ground card now 14 "+20 vs Mechanical"** via static `DisplayEffect`→`TwinGatlingCannonsWoLU` repoint (CHECK11-gated); 🟡 cloak-on-spawn (0.3.11 — not normally cloak-capable; `PersonalCloakingFree` on both modes, no toggle/decloak button, like Reaper/Medivac); ⛔ Wild Missiles (deferred — swaps the air primary attack, owner-deferred) |
...
| **Hel's Angels** | Viking | +45% HP, +40% dmg | ✅ SEPARATE ids `HelsAngel{Fighter,Assault}` → explicit parity edits (+55% speed; **free permanent cloak, both modes, 0.3.11**). Calldown cost unchanged — base Viking has no cost cut. **0.3.13: ground projectile/impact restored** (own `WreckingCrewAssaultAttackWoLU` actor clone — the `WreckingCrewAssault` Effect reroute had detached it) + **ground card now 14 "+20 vs Mechanical"** (its weapon has no `DisplayEffect` → panel falls back to the already-rerouted `<Effect>` → reads the player value directly; CHECK11 leaves it alone). Weapon scaling reaches it via the vanilla `TerranShipWeaponsLevel{N}` `Effect,WreckingCrewAssault,Amount` entry our sync grants |
...
- **Senior / regular Ghost +50% dealt damage** — `DamageDealtFraction` rewritten to the correct indexed-per-Kind form. The card still shows weapon base (20) — a damage-*dealt* buff never changes the displayed weapon number, and the Ghost weapon is SHARED, so making the panel exact would need a **Senior-Ghost-specific weapon clone** (owner-decision-pending). Confirm the *dealt* damage is ~30 in game.
