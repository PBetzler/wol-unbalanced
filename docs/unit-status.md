...
> **Last updated: 2026-06-19 (v0.3.13 — flier upgrade reach, reroute-detach sweep, panel truth, Thor-family AA barrage).**
>
...
| **Engineering Bay combined upgrades** | One Atk + one Def upgrade per tier, infantry cost, 30 s each; applies to infantry/vehicles/ships (vehicle/ship levels follow infantry via periodic sync) | **0.3.12 FIX:** the sync detected the base `TerranInfantryWeaponsLevel{N}` ids, but the campaign's EngBay grants `…UltraCapacitorsLevel{N}` / `…VanadiumPlatingLevel{N}` → the condition never matched → vehicles/fliers never upgraded. Sync now OR-detects the wrapper ids. **0.3.13 FIX:** (a) the earlier flier-coverage gap is CLOSED — base Banshee/BC/Viking (air+ground) weapons + Banshee/BC/Viking/Medivac/Raven/Thor/SiegeTank/Hellion armor + elite-merc armor are now appended into the base `Terran{Ship,Vehicle}{Weapons,Armors}Level{N}` EffectArrays so the sync's grant lifts ALL player units; (b) the 30 s cap extended to the after-lab Research11–16 buttons (holds in both lab states); (c) the `diagUpg` subtitle now prints `infW=base/uc` (BEFORE-lab `1/0`, AFTER-lab `0/1` — the old single-id read falsely implied the sync hadn't fired). ✅ statically; 🟡 owner confirms via `diagUpg` + that a Banshee/BC/Viking now gains armor/weapon levels when Eng Bay upgrades complete (one in-game caveat: whether the `Effect,TwinGatlingCannons,Amount` bump reaches the rerouted Viking-ground clone — same class as the GAME-confirmed Thor case) |
...
| Unit | Base baseline | What the mod changes | Status |
| --- | --- | --- | --- |
| **Hellion** | 100/0, 2 supply | All AP except Jump Jets; Twin-Linked Flamethrower; Thermite Filaments; +2 armor (Infernal Plating); Smart Servos; Super Stim (60-heal); OL build-time −25% (30→22 s); **now scales armor with the combined Eng Bay upgrade** (0.3.13 added base Hellion armor to the base-Level upgrades) | ✅; 🟡 [GAME] confirm Hellion armor rises with Eng Bay Def upgrades |
...
| **Siege Tank** | 150/125, 3 supply | All AP except Jump Jets / Internal Tech / move-while-sieged; Maelstrom Rounds; **Shaped Blast → no friendly splash** (`CrucioShockCannonFriendly*Damage=0`); Graduating Range (+1/3 s, 5 stacks); +3 armor sieged; Spider Mines; Laser Targeting; Resource Efficiency; **now scales armor with the combined Eng Bay upgrade** (0.3.13 added base Siege Tank armor to the base-Level upgrades) | ✅ no-FF + LTS proven; 🟡 Graduating Range (confirm 5 stacks = +5 range); 🟡 Spider Mine fixes (own hotkey Y; sieged deploy via raw `SpawnSpiderMine`); 🟡 [GAME] confirm armor scaling |
| **Thor** | 300/200, 6 supply | Goliath upgrade package + AP minus nuke/field-construction/HIP-mode; **ground splash = sieged-tank** (`ThorsHammerWoLU` effect-override); **AA = 35/rocket flat** (HIP floor, no light bonus), AA range 12 (HIP+1); **AA barrage 0.3.13: 8 missiles per attack (was 4) at ~2/3 cadence** (`PeriodCount` 4→8, `Period` ×0.6667; `DisplayAttackCount` ×2 keeps the card "×8" honest); Rapid Reload (Period 1.93→1.0); Laser Targeting; **now scales armor with the combined Eng Bay upgrade** (0.3.13 added base Thor armor to the base-Level upgrades); simultaneous air+ground fire (`LinkedCooldown=0`) | ✅ ground splash + Rapid Reload proven; ✅ AA barrage doubling + 2/3 cadence statically proven (CHECK8-GOOD scalar `PeriodCount`/`Period`/`DisplayAttackCount`), 🟡 [GAME] barrage feel; ✅ **AA card now flat 35** (no phantom "+4 vs Light") via static `DisplayEffect`→`JavelinMissileLaunchersDamageBuffedWoLU` repoint (CHECK11-gated); 🟡 AA splash (Shaped-Blast clone — confirm radius); 🟡 simultaneous fire (global `Options` edit — accepted enemy leak); **ground muzzle/impact + AA impact SOUND RE-FIXED 0.3.31 (Trigger-Override)** — the earlier action-clone re-binds (`ThorAttack{Vanilla,}WoLU`, `ThorAAAttack{Vanilla,}WoLU`) never took in-game (owner saw nothing v0.3.11→0.3.30), so they were DELETED; the reroute sets now re-include the UN-gated vanilla effects (`ThorsHammerDamage` / `JavelinMissileLaunchersDamage`) as member[0] → the untouched vanilla `ThorAttack`/`ThorAAAttack` keep firing the muzzle/impact. Minor on-theme side effect: the player's primary target takes the vanilla base hit AND the buffed splash (≈ partial double). 🟡 [GAME]-pending owner re-confirm |
...
| Unit | Base baseline | What the mod changes | Status |
| --- | --- | --- | --- |
| **Medivac** | 100/100, 2 supply | All AP; +100 shields (Scatter Veil); +4 cargo (`MedivacTransport` 8→12); Afterburners; permanent cloak; Rapid Deployment; Advanced Healing AI; Resource Efficiency; **now scales armor with the combined Eng Bay upgrade** (0.3.13 added base Medivac armor to the ship-armor base-Level upgrades) | ✅ stats (cargo=12 is the proven class); 🟡 cloak-on-spawn (no toggle — periodic buff only); 🟡 [GAME] confirm armor scaling |
...
| **Viking** | 150/75, 2 supply | All AP; Ripwave; Phobos (+range); **Shredder line-splash** + Anti-Mech (Assault mode); +55% speed (Aesir Turbines); Smart Servos; **free permanent cloak (0.3.11, owner request — both modes)**; **now scales (air+ground) weapons + armor with the combined Eng Bay upgrade** (0.3.13 added base Viking entries to the base-Level upgrades) | ✅ ground splash + anti-mech; **ground projectile/impact RE-FIXED 0.3.31 (Trigger-Override)** — the 0.3.13 action-clone re-bind (`VikingAssaultAttack{Vanilla,}WoLU`) never took in-game (owner still saw no muzzle/launch flash/projectile/impact through v0.3.30), so it was DELETED; `TwinGatlingCannonsWoLUSet` now re-includes the UN-gated vanilla `TwinGatlingCannons` as member[0] → the untouched vanilla `VikingAssaultAttack` keeps firing the launch sound + impact sound + blood. Minor on-theme side effect: primary ground target takes the vanilla base hit AND the buffed splash. 🟡 [GAME]-pending owner re-confirm; ✅ **ground card now 14 "+20 vs Mechanical"** via static `DisplayEffect`→`TwinGatlingCannonsWoLU` repoint (CHECK11-gated); 🟡 cloak-on-spawn (0.3.11 — not normally cloak-capable; `PersonalCloakingFree` on both modes, no toggle/decloak button, like Reaper/Medivac); ⛔ Wild Missiles (deferred — swaps the air primary attack, owner-deferred) |
| **Banshee** | 150/100, 3 supply | All upgrades; +2 range / +25% dmg while cloaked (folded flat since perma-cloaked); Rocket Barrage; Shockwave Battery; Hyperflight (+25% speed); Laser Targeting; free permanent cloak; **now scales weapon + armor with the combined Eng Bay upgrade** (0.3.13 added base Banshee entries to the ship base-Level upgrades) | ✅ stats; 🟡 cloak-on-spawn; 🟡 [GAME] confirm weapon/armor scaling |
| **Battlecruiser** | 400/300, 6 supply, 110 s build | **Build 110→60 s**; all AP; **fire while moving** (`AllowedMovement=Moving`); **ignore armor** (`ArmorReduction=0` — was backwards at 500, fixed); autocast Missile Pods + Defensive Matrix + **Yamato** (big-target-gated, skips structures); +2 armor; +25% speed; Tactical Jump; **now scales (air+ground) weapons + armor with the combined Eng Bay upgrade** (0.3.13 added base BC `ATSLaserBattery`/`ATALaserBattery` + armor to the ship base-Level upgrades) | ✅ ignore-armor + autocasts shipped (kit clones `parent=`); 🟡 Yamato structure-discrimination (skips ALL structures — refinement deferred); 🟡 [GAME] confirm weapon/armor scaling; ⛔ smart-batteries (rogue-kit blueprint exists, single-target fallback works — not shipped blind) |
| **Raven** | 100/200, 2 supply | All AP except Hunter-Seeker-as-attack; **summons permanent** (PDD/Auto-Turret Duration 3600); **free kit** (PDD/Turret/Seeker energy → 0, player-only); **self Point-Defense Laser (0.3.21)** — `PointDefenseLaserWoLU` on the Raven's `WeaponArray`, `WoLUHaveFlag`-gated so ONLY the player's Raven fires it (1 energy/intercept, missile-only, no friendly-fire); Interference Matrix; Anti-Armor Missile; **free permanent cloak (0.3.12)**; **now scales armor with the combined Eng Bay upgrade** (0.3.13 added base Raven armor to the ship-armor base-Level upgrades) | ✅ free-kit + permanent summons (owner's rule-9-safe design); 🟡 [GAME] confirm armor scaling; 🟡 [GAME] confirm the self-PDD laser fires from the Raven's turretless mount + that enemy Ravens don't intercept (fallback: `Turret="PointDefenseDrone"`); ✅ superseded the old "owner rejected a hidden always-on weapon" — now armed globally but flag-gated to the player |
| **Science Vessel** | 100/200, 2 supply | All AP; **one free double-beam Nano-Repair (0.3.22)** — heals **two targets at once**, **biological + mechanical + air** (transplanted Medivac Advanced-Healing-AI beam-count mechanism, `HealBeamAvailable` LT-2 gate); **autocast EMP Shockwave (0.3.21)** (only enemies with energy/shields); **autocast Irradiate (0.3.21)** (enemy biological); Defensive Matrix (clone, autocast); Tactical Jump; **free permanent cloak (0.3.12)** | ✅ static (clones gate-green); 🟡 [GAME] confirm the double-beam heals 2 at once + free-sustains at 0 energy; EMP/Irradiate autocast targeting; the heal beam visual may render as the Medivac medic beam (card icon stays Nano-Repair — parked, no actor guessed) |
...
| **Odin** | Thor | AA 35/rocket + range 12; **AA barrage 8 missiles per attack at ~2/3 cadence (0.3.13, rule 10 — own `OdinAntiAir`/`OdinAA` chain)**; ground range +1, sight +2; Rapid Reload (Period 1.0); simultaneous fire | ✅ (mirrors Thor; global `Options` edit accepted); ✅ AA barrage doubling statically proven (CHECK8-GOOD `OdinAA.PeriodCount` 4→8 + `OdinAntiAir.Period` ×0.6667 + `DisplayAttackCount` ×2), 🟡 [GAME] barrage feel. (Odin's ground weapon `Odin` was never rerouted → its muzzle never broke; AA card-number/impact-restore fixes are on the shared Thor chain, not Odin's) |
...
| **Hel's Angels** | Viking | +45% HP, +40% dmg | ✅ SEPARATE ids `HelsAngel{Fighter,Assault}` → explicit parity edits (+55% speed; **free permanent cloak, both modes, 0.3.11**). Calldown cost unchanged — base Viking has no cost cut. **ground projectile/impact RE-FIXED 0.3.31 (Trigger-Override)** — the 0.3.13 action-clone re-bind (`WreckingCrewAssaultAttackWoLU`) never took in-game, so it was DELETED; the merc weapon `WreckingCrewAssault` now points at a SET `WreckingCrewAssaultWoLUSet` that re-includes the un-gated vanilla `WreckingCrewAssault` as member[0] → the untouched vanilla `WreckingCrewAssaultAttack` keeps firing the launch/impact. Minor on-theme side effect: target takes the vanilla base hit AND the splash clone. 🟡 [GAME]-pending owner re-confirm. **Ground card 14 "+20 vs Mechanical"** — weapon has no `DisplayEffect` → panel falls back to `<Effect>` (now the set, both members un-gated) → CHECK11 reports "no flag-gated damage clone", leaves it alone. Weapon scaling reaches it via the vanilla `TerranShipWeaponsLevel{N}` `Effect,WreckingCrewAssault,Amount` entry our sync grants |
...
| **Jotun** (`MercThor`) | Thor | Thor merc; inherits Thor (Laser Targeting + Shaped Hull at idx 10/11); **inherits the 0.3.13 AA barrage (8 missiles, 2/3 cadence) + armor scaling for free** (parent=Thor, shared `JavelinMissileLaunchers`/`JavelinMissileLaunchersPersistent` ids + explicit `MercThor` armor entry) | ✅ shows inherited Thor cards; Vehicle life-armor + shield signs resolve; ✅ windup capped (both weapons' `DamagePoint`≤0.1 inherited) + simultaneous-fire `Options[]` (inherited) — both [STATIC], 🟡 [GAME] confirm; ✅ AA barrage doubling inherited (CHECK8-GOOD), ✅ AA card flat 35 + AA impact sound restored (shared Thor chain, CHECK11-gated); **Immortality Protocol now WORKS v0.3.9** — parallel `MercThor{DontDie,Wreckage,Reborn}` chain revives AS A JOTUN, no gas, single HP bar (own wreck actor); 🟡 [GAME] confirm rebuild identity/no-gas; 🟡 AA splash; **ground muzzle/impact restored 0.3.11** (🟡 [GAME] confirm) | 🟡 `ThorPortrait` |
...
| **Senior Ghost** (`MercSeniorGhost`) | Ghost | Ghost clone on the **Ghost** model (not Nova); **1.5× life/energy/regen**; +50% dmg (rifle baked into `C10CanisterRifleWoLU`, 30/+15 Light); flat armor | ✅ shows inherited Ghost card (Laser Targeting); Infantry life-armor + shield signs resolve; **rifle muzzle/impact RESTORED v0.3.31** (weapon `<Effect>` → Trigger-Override set `C10CanisterRifleWoLUSet` re-includes vanilla `C10CanisterRifle` as member[0] → vanilla `GhostAttack` action fires again; 🟡 [GAME] confirm). **Owner-approved damage increase:** target now takes the base hit (~20/+10 Light) AND the +50% clone (30/+15 Light); 🐞 #10 "two stim buttons" not statically reproducible (one stim button in merge sim) | 🟡 `GhostAlternate` model / `GhostPortrait` |
...
**RESOLVED (0.3.13) — the two CHECK11-gated panel-number divergences on rerouted weapons:**
- **Thor/Jotun AA card** now reads **flat 35** (no phantom "+4 vs Light"). The panel reads the weapon's `DisplayEffect`, which was vanilla-pinned to `JavelinMissileLaunchersDamage` (per-player `Amount`=35 but `AttributeBonus[Light]`=4 unchanged), while the player actually fires the flat-35 `JavelinMissileLaunchersDamageBuffedWoLU`. Fixed by a static `DisplayEffect` repoint to that buffed clone (`WeaponData.xml`; rule-9-safe — display-only). CHECK11 gates it.
- **Viking/Hel's Angels ground card** now reads **14 "+20 vs Mechanical"**. The base Viking weapon's `DisplayEffect` was pinned to `TwinGatlingCannons` (14, no bonus) while the player fires `TwinGatlingCannonsWoLU` (14 + `AttributeBonus[Mechanical]`=20) — repointed to the clone (`WeaponData.xml`). (The Hel's Angels variant had no `DisplayEffect` → panel falls back to its already-rerouted `<Effect>`, so it was already correct.)

These are the **still-unresolved** questions whose root cause is the **per-player indexed-array edit** (suspected no-op on the *displayed* card number, even when the *value* is correct) — both owner-decision-pending:

- **"X dmg + Y vs light/armored" → "X flat"** (Ghost `C10CanisterRifle`, Spectre `SpecterU`). The effect/field paths are CONFIRMED correct, but a per-player `AttributeBonus[Light/Armored]` edit is a confirmed NOOP class (CHECK8) → the player's rifle still shows/deals the +vs bonus. The only fix that removes it for the player (not enemies) is a **Shaped-Blast effect clone** (recipe 13), same pattern as the Jotun AA splash — owner decides whether to build it. (The Thor/Jotun AA case in this family is now DONE — value 35 + splash via clone + the card-number repoint above.)
- **Senior Ghost +50% dealt damage — RESOLVED (v0.3.x panel clone + v0.3.31 rifle-action restore).** The merc fires the `MercSeniorGhost`-only weapon clone `C10CanisterRifleWoLU` (`Amount=30`, `+15 Light` = the player's 20/+10 × 1.5 baked), whose `<DisplayEffect>` is the same clone, so the panel reads the real 30. v0.3.31 converted the weapon's `<Effect>` to the Trigger-Override set `C10CanisterRifleWoLUSet` (re-includes vanilla `C10CanisterRifle` as member[0]) to RESTORE the detached `GhostAttack` rifle muzzle/impact — owner-approved the resulting damage increase (target takes base ~20/+10 Light + the 30/+15 Light clone). `🟡 [GAME]` confirm the rifle muzzle/impact + dealt damage. (The *regular* Ghost rifle "+vs Light" flatten remains the separate owner-decision item below — base Ghost is untouched by this merc-only fix.)
