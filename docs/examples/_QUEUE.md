...
| ✅ | [18](18-elite-merc-clone.md) | Previewer renders the elite mercs | **VERIFIED 2026-06-17 (Editor, full-deps map traynor01):** MercThor (Jotun) → body model **"Thor"** + portrait **"Portrait - Thor"** (real Thor portrait, NOT a heart; renders, NOT a sphere). Condor renders as a Hellion. **All 6 elite mercs present + resolve** with full deps, incl. Winged Nightmares (MercWraith → Wraith weapons/cloak, Life 200/Shields 50/armor 1 = the static values). The earlier "MercWraith missing" was a **standalone-mod artifact**, now resolved (open-issues.md). Mechanism proven; CHECK6 confirms all 6 portrait tokens. |
