...
## v0.3.31 — Jotun/Thor + Viking ground-attack visuals RE-FIXED via Trigger-Override (the action-clone re-bind never took) (2026-06-22)

**Root cause of the long-standing "ground attack plays the animation but shows no muzzle/launch flash, no projectile, no impact" defect was the WRONG FIX, not a missing one.** Since v0.3.11 the repo tried to restore the detached muzzle/impact by CLONING the vanilla `CActorAction` (`parent="<vanilla action>"` + a re-specified `effectAttack`/`effectImpact` pointing at the rerouted child effect id). That re-bind does NOT take effect in-game — it shipped v0.3.11 → v0.3.30 and the owner STILL saw nothing on the Jotun/Thor + Viking ground attacks. The working mechanism is the **Trigger-Override shape** the Wraith sets already used (`WraithGTriggerOverrideSet` keeps the vanilla `WraithGLaunchMissile` as member[0]).

- [x] **Thor/Jotun ground muzzle+impact, Thor/Jotun AA impact sound, Viking ground launch/impact/blood, Hel's Angels ground launch/impact — ALL RE-FIXED (v0.3.31, [STATIC]; [GAME] confirm pending).** For each of the 4 reroute sets, re-included the UNMODIFIED vanilla delivery/damage effect as member[0] so the bare id STILL FIRES and the UNTOUCHED vanilla `CActorAction` keeps matching: `ThorsHammerWoLUSet` += `ThorsHammerDamage`; `JavelinMissileLaunchersDamageWoLUSet` += `JavelinMissileLaunchersDamage`; `TwinGatlingCannonsWoLUSet` += `TwinGatlingCannons`; converted the Hel's Angels bare-clone reroute into a SET `WreckingCrewAssaultWoLUSet` = `[WreckingCrewAssault (vanilla), WreckingCrewAssaultWoLU]` and pointed the weapon `<Effect>` at it. DELETED the redundant `*VanillaWoLU` effect clones (`ThorsHammerDamageVanillaWoLU`, `TwinGatlingCannonsVanillaWoLU`, `JavelinMissileLaunchersDamageVanillaWoLU`) and the 7 dead `CActorAction` clones (`ThorAttack{Vanilla,}WoLU`, `ThorAAAttack{Vanilla,}WoLU`, `VikingAssaultAttack{Vanilla,}WoLU`, `WreckingCrewAssaultAttackWoLU`). The vanilla `CActorAction`s (`ThorAttack`/`ThorAAAttack`/`VikingAssaultAttack`/`WreckingCrewAssaultAttack`) are left UNTOUCHED. Files: `EffectData.xml`, `WeaponData.xml`, `ActorData.xml`. **⚠ [GAME] confirm the Jotun/Thor + Viking ground attacks now show muzzle/launch flash + impact + blood, and Thor/Jotun AA hits play their impact sound.**
  - **Accepted on-theme side effect (NOTE FOR THE OWNER):** the un-gated vanilla member means the PLAYER's primary target takes the vanilla base hit AND the buffed splash clone — a damage increase / partial double on the primary. Acceptable per "balance is a non-goal / funnily overpowered"; enemies are unchanged (the un-gated member is exactly their vanilla damage → rule-9-neutral). No ring-exclusion added (deliberately — keeps it simple).
  - **CHECK11 stays green:** it compares only the `WoLUHasFlag`-gated clone to the panel and ignores the un-gated vanilla member; the Hel's Angels set has no flag-gated member so it reports "vanilla delivery — no damage clone" (no divergence) — verified by running the gate.
- [x] **Senior Ghost rifle muzzle/impact detach — RESOLVED (v0.3.31 Trigger-Override, owner-APPROVED the damage increase; [STATIC], [GAME] confirm pending).** The `C10CanisterRifleWoLU` weapon (on `MercSeniorGhost`) had fired the NEW effect id `C10CanisterRifleWoLU`, which did NOT match `GhostAttack`'s `effectAttack="C10CanisterRifle"` → the Senior Ghost's rifle muzzle/impact sound + blood (`Ghost_AttackLaunch`/`Ghost_AttackImpact` + bunker-launch + `BloodTargetImpact`) detached (hitscan rifle, no missile/beam to save it). **Fix:** the weapon's `<Effect>` now points at the Trigger-Override set `C10CanisterRifleWoLUSet = [C10CanisterRifle (vanilla, un-gated — re-attaches the GhostAttack action + delivers the player-resolved base ~20/+10 Light), C10CanisterRifleWoLU (the existing +50% clone, 30/+15 Light)]` (`EffectData.xml`/`WeaponData.xml`). `<DisplayEffect>` stays `C10CanisterRifleWoLU`. **Accepted side effect (owner-approved):** the merc's primary target takes the base hit (~20/+10 Light) ON TOP of the +50% clone (30/+15 Light) — a damage increase that stacks on the accuracy fix; balance is a non-goal. Player-exclusive merc (no flag gate). CHECK11 treats the un-gated set as "vanilla delivery — no damage clone" and skips the panel compare (no `WoLUHasFlag` member) — verified green. Base Ghost / Nova / Tosh are UNAFFECTED (base Ghost fires vanilla `C10CanisterRifle` → `GhostAttack` matches; Nova fires the SEPARATE `Nova` weapon→effect, not `C10CanisterRifle`). No stray `GhostAttack`/`C10CanisterRifle*WoLU` `CActorAction` exists in our ActorData (verified). See docs/learnings.md (reroute-universe table + the Senior Ghost bullet). **⚠ [GAME] confirm the Senior Ghost rifle now shows muzzle/impact flash + blood.**
- Confirmed clean (no change): **Odin / OdinAntiAir** were never `<Effect>`-rerouted (only `Options` added) → never detached; the **Wraith** sets already use the Trigger-Override shape; the **SV EMP** binds by `AmmoUnit` unit-name.
