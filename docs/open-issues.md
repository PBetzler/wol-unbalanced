...
- [~] **Spartan Companies can't fire (at least their AA missiles) while loaded in a Bunker.** Our mod lets mechanical/size-8 units load (open-issues #2/#3). **CANDIDATE SHIPPED v0.3.11 (owner decision):** per-player `Weapon.Arc=360` on the 4 SpartanCompany weapons (`SpartanCompany{A,G,AUpgraded,GUpgraded}`, genlib) — CHECK8 GOOD scalar, rule-9-safe (enemy Goliaths keep 29.99°). ⚠ **[GAME] owner playtests** whether a bunkered Spartan then fires air; if it still won't, it's an inherent SC2 turret-from-bunker limitation → revert/accept.
