...
> **Still OPEN (owner-decision-pending) — the remaining damage-display items in this family** (see §"Damage display + values" below): Ghost/Spectre rifle "+vs Light/Armored" flatten (a per-player `AttributeBonus` edit is a confirmed NOOP class → needs a Shaped-Blast effect clone, recipe 13) and Senior Ghost +50% panel under-show (the Ghost weapon is SHARED → needs a Senior-Ghost-specific weapon clone to make the panel exact). Neither is a reroute-detach or a CHECK11 case; both await an owner call on whether to build the clone.
