...
- [ ] **Each elite merc shows its OWN name** when selected — "Jotun", "Winged Nightmares", "Skibi's Angels", "Senior Ghost", "Brynhilds", etc. — NOT the parent's ("Thor"/"Wraith"/"Medic"/"Ghost"). *(Fix: explicit `<Name>` field.)*
- [ ] [RISK] **Brynhilds (Valkyrie merc, v0.3.2)** appears as a 7th button in the elite-merc submenu (R1C2). Calls down 2 anti-air flyers wearing the **Valkyrie model** (NOT a sphere — the model is base-CASC). If it's a sphere, the "Valkyrie" model token doesn't resolve in CASC; report it.
...
- [ ] **(v0.3.14) Skip Prophecy (Zeratul) missions button** — tested on the **Hyperion hub** (`tstory01`), reachable once **Tychus 02** is complete (which unlocks the Zeratul/Prophecy arc) and BEFORE finishing all four Prophecy missions. Expect a top-center on-screen button "**Skip Prophecy Missions (grant research)**". Confirm: (1) it appears only in that unlocked-but-unfinished window (not before Tychus 02, not after all 4 are done, and NOT on regular mission maps); (2) one click marks **all 4** Zeratul missions complete (they show cleared on the star map / mission list) and the **Lab research-point total jumps** by the Prophecy missions' full 100%-clear yield (Protoss + Zerg); (3) the button **disappears** on click and **stays gone** after leaving and re-entering the hub (one-time, persisted); (4) the on-screen **placement** (top-center, +120 px) is acceptable (easy to move if not). If the research total doesn't move, the suspect is the opportunity-vs-reward array split (see [open-issues.md](open-issues.md) §v0.3.14 / [learnings.md](learnings.md)); if the missions don't mark complete or downstream stays locked, the suspect is the `UpdateMissionFlow()` / `PrepareForStoryModeTransitionalSave()` persistence path.
