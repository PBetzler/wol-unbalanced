...
| Midnight Riders | Mercenary Liberators | Wraith + **Liberator model** | +75 sh, 250 HP, +4 range, +2 vis, cloak, drops 2 |
| Senior Ghost | (special) | Ghost + **Nova model** | 1.5× life/energy/regen/dmg, +3 armor, +75 sh, drops 1 |

- [ ] [RISK] **Midnight Riders renders as a Liberator** (Wraith clone wearing the `Liberator` model) — if it looks like a Wraith or is invisible, the `Model="Liberator"` token didn't resolve. It's a reskin (no real Defender-Mode transform), cloaked, +4 range.
- [ ] [RISK] **Senior Ghost renders as Nova** (`Model="Nova"`) with **1.5× stats** — bigger life/energy bar, hits ~50% harder, +3 armor; keeps Snipe/EMP/cloak. If damage looks unchanged, the `DamageDealtFraction=0.5` behavior didn't read as +50% (the one schema guess here).
